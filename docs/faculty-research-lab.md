...
[Research on DSMLP](individual-researcher.md) covers researchers working on
their own. [Teaching with Datahub and DSMLP](instructor-or-ta.md) covers course
setup.
...
workloads of that shape to a more appropriate platform. See
...
| A lab workspace with its own quota | Guaranteed access up to the group's quota, date-aware so that it can flex around the group's deadlines | None for compute. [Cost](#cost) describes storage charges. |
| Contributing hardware (pilot) | Exclusive reservation rights over the contributed capacity | Purchase of the hardware. See [Contributing Hardware (Pilot)](#contributing-hardware-pilot) |
...
sets out storage charges.
[Self-Supporting Programs](reference/policy.md#self-supporting-programs) covers
cost recovery for work within a self-supporting program.

## Establishing a Lab Workspace

A workspace anchors a group's roster, storage, container images, GPU access,
and storage and GPU quotas. See
...
[Managing a Group](reference/managing-a-group.md) describes privilege tiers.
...
containers. Capacity thresholds govern what is mounted where. See
...
describe sharing with collaborators inside and outside UC San Diego, through
group permissions and public access.

### Data Classification

Data classified P4, such as clinical records or export-controlled information,
may not be used on DSMLP. Data classified P3 may be permitted after review.
[Data Classification](reference/policy.md#data-classification) describes the
review and gives the vetting period. Raise either classification in the initial
...
GPUs are allocated in five size classes. See
...
describes borrowing, including how far ahead it applies.
...
mechanism that courses use to divide capacity across a roster. See
...
Budgets are set administratively, and changes are requested by ticket. See
[Administrative Requests](reference/getting-help.md#administrative-requests).
...
gone. [Reservation Events](reference/reservation-events.md) defines each event:
...
The standard software images suffice for most groups.
...
standard image where possible, which is the supported path. See
...
inside the container, which constrains how dependencies are installed. See
...
requires, and the reclaiming of an idle GPU session. See
...
- Checkpoint long-running work. Runtime limits, idle culling, and preemption all
  end containers for reasons unrelated to the code they run. See
...
completed before that window.
[Scheduled Maintenance](reference/policy.md#scheduled-maintenance) gives both
schedules.
...
departing member. Establish what becomes of a departing member's files.
[When Access Starts & Ends](access/when-access-starts-and-ends.md) describes the
end of access.

## Policy

[Policy](reference/policy.md) sets out conditions of use, including data
classification, appropriate use, hosting of externally available services, and
...
response targets. [Getting Help](reference/getting-help.md) lists all support
routes.
